...
Namespace MathEx.AccelMethods
    ...
    Public Class GDEM
        ...
        Public Shared Function Promote(ByVal currX As Double(), ByVal dXvec As ArrayList, ByVal nu As Integer) As Double()
            ...
            If nu > 1 Then
                ...
                Do
                    ...
                    Do
                        m2 = dXvec(m - k)
                        b(j - 1, k - 1) = WeightedProductNorm(m1, W, m2) ^ 2
                        k = k + 1
                    Loop Until k = nu
                    j = j + 1
                Loop Until j = nu
                ...
                Do
                    m1 = dXvec(m - j)
                    m2 = dXvec(m - 1)
                    b1(j, 0) = WeightedProductNorm(m1, W, m2) ^ 2
                    j = j + 1
                Loop Until j = nu - 1
            ...
            Else
                ...
                mu(0, 0) = -WeightedProductNorm(m1, W, m2) ^ 2

                m1 = dXvec(m - 1)
                m2 = dXvec(m - 1)

                mu(0, 0) = mu(0, 0) / WeightedProductNorm(m1, W, m2) ^ 2
        ...
        Private Shared Function WeightedProductNorm(ByVal m1 As Mapack.Matrix, ByVal W As Mapack.Matrix, ByVal m2 As Mapack.Matrix) As Double
            Return Mapack.Matrix.Multiply(Mapack.Matrix.Multiply(m1.Transpose(), W), m2).FrobeniusNorm
        End Function
