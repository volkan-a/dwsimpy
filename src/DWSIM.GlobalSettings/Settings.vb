...
Public Class Settings
    ...
    Public Shared Sub InitializePythonEnvironment(Optional ByVal pythonpath As String = "")
        Throw New PlatformNotSupportedException("Python.NET initialization is not part of the pure .NET 10 GlobalSettings runtime.")
    End Sub
    ...
    Shared Sub LoadExcelSettings(Optional ByVal configfile As String = "")
        ...
        If configfile = "" Then configfile = Path.Combine(GetApplicationDirectory(), "dwsim.ini")

        If Not File.Exists(configfile) Then File.Copy(Path.Combine(GetApplicationDirectory(), "default.ini"), configfile)
    ...
    Shared Sub SaveExcelSettings(Optional ByVal configfile As String = "")
        ...
        If configfile = "" Then
            configfile = Path.Combine(GetApplicationDirectory(), "dwsim.ini")
            File.Copy(Path.Combine(GetApplicationDirectory(), "default.ini"), configfile, True)
        Else
            File.Copy(Path.Combine(GetApplicationDirectory(), "excelcompat.ini"), configfile, True)
        End If
    ...
    Private Shared Function GetApplicationDirectory() As String
