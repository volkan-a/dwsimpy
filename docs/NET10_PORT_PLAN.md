...
sensitivity-analysis classes, together with bulk and distillation-curve
petroleum assay data. Optimization, sensitivity, and petroleum assay
collections now use their concrete DWSIM types. Their legacy `BinaryFormatter`
clone paths were replaced with explicit deep copies, while expression compiler
state is kept as solver-owned runtime data without a `Ciloci.Flee` reference.
The old editor forms, resource bitmap wrappers, update checks, weather
providers, IronPython snippets, and desktop helpers remain outside the pure
runtime boundary.
...
optimization, sensitivity-analysis, and petroleum assay surface.
...
2. **Create SDK-style net10 projects**
   ...
   - Continue expanding the remaining headless contract/data classes:
     - `DWSIM.SharedClasses` charts, GHG emission summary, and runtime utility
       methods
