...
data event args, weather data, exception processing, optimization, and
sensitivity-analysis classes. Optimization and sensitivity collections now use
their concrete DWSIM types. Their legacy `BinaryFormatter` clone path was
replaced with explicit deep copies, while expression compiler state is kept as
solver-owned runtime data without a `Ciloci.Flee` reference. Petroleum assay
collections remain headless object collections until those concrete data types
are ported. The old editor forms, resource bitmap wrappers, update checks,
weather providers, IronPython snippets, and desktop helpers remain outside the
...
options/results, transition restore, weather data, exception processing,
optimization, and sensitivity-analysis surface.
...
2. **Create SDK-style net10 projects**
   ...
   - Continue expanding the remaining headless contract/data classes:
     - `DWSIM.SharedClasses` charts, petroleum assay data, GHG emission summary,
       and runtime utility methods
